    ...
    return (
      <div className="card">
        <div className="card-body form-inline">
          ...
          <select
            className="custom-select timeChoicePick ml-2"
            value={this.state.timeChoice}
            name="timeChoice"
    ...
    return (
      <div>
        {!dataShown && !timeChoiceFilled && (
          <ChooseTimeAndDiff handleApply={this.applyChange} />
        )}
        <div className="testArea">
          {!dataShown && timeChoiceFilled && (
            <Tester
              testHandler={this.testHandler}
              timeChoice={this.state.timeChoice}
              training={false}
              diffu={this.state.diffu}
              changeTimeHandler={this.changeTimeHandler}
            />
          )}
          {dataShown && timeChoiceFilled && (
            <DataVis
              speedOverTime={this.state.speedOverTime}
              accOverTime={this.state.accOverTime}
              errorKeys={this.state.errorKeys}
              wordErrors={this.state.wordErrors}
            />
          )}
          {dataShown && (
            <button
